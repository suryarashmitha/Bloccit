...
describe('routes : posts', () => {
	...
	describe('guest user performing CRUD actions for posts', () => {
      ...
      it("should NOT render a new topic form, but redirect to topic page", (done) => {
        request.get(`${base}/${this.topic.id}/posts/new`, (err, res, body) => {
          expect(err).toBeNull();
          expect(body).toContain("Winter Games");
          expect(body).toContain("View Topic");
          done();
        });
      });
	...
	describe('ADMIN performing CRUD actions on posts', () => {
		beforeEach(done => {
			User.create({
				...
				request.get(
					{
						url: 'http://localhost:3000/auth/fake', // mock authenticate as admin user
						form: {
							role: user.role,
							userId: user.id,
							email: user.email,
